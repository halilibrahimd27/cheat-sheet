...
> `attack.mitre.org`, and 82% of commands carry a reference chip linking to that tool's
> own documentation.
...
- **Reference links** — **4,139 of the 5,040 commands (82%) carry a reference chip** to the tool's own documentation: 4,140 links across 96 hosts, every one of them fetched and confirmed to answer 200 before it shipped. The link is the vendor's or the project's own page — never a blog, a shortener or an SEO aggregator — and a forge link labels itself with the project rather than with "github.com". The rest are shell builtins and payload snippets, where no honest single source exists; `node scripts/validate-content.js` ratchets that gap so it can shrink but never grow
