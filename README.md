...
- **MITRE ATT&CK tags** — commands can carry optional technique ids (e.g. `T1558.003`) shown as chips linking to attack.mitre.org, with a **sidebar ATT&CK facet** that appears only when the data uses them; commands can also carry **reference links** (HackTricks, GTFOBins, docs)
